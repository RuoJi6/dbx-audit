            ...
            let mut scans = stream::iter(targets)
                .map(|target| {
                    ...
                    async move {
                        ...
                        scan_audit_table(state, request, engine, job_id, target).await.map(Some)
                    }
                })
...
async fn scan_audit_table(
    ...
    if request.mode.includes_content() && !is_cancelled(&job_id) {
    ...
    let table_result = if is_cancelled(&job_id) {
        None
    } else {
...
async fn scan_table_content_findings(
    ...
    loop {
        if is_cancelled(job_id) || max_rows.is_some_and(|max| scanned >= max) {
            break;
        }
...
async fn audit_target_schemas(
    ...
    let schemas = dbx_core::schema::list_schemas_core(state, &request.connection_id, database)
        .await
        .unwrap_or_default()
        .into_iter()
...
async fn effective_field_worker_count(state: &AppState, request: &AuditScanRequest) -> usize {
    match connection_db_type(state, &request.connection_id).await {
...
async fn audit_sql_dialect(state: &AppState, connection_id: &str) -> Option<AuditSqlDialect> {
    ...
    match db_type {
        ...
        | DatabaseType::Gbase => Some(AuditSqlDialect::Mysql),
        ...
        DatabaseType::Sqlite | DatabaseType::Rqlite => Some(AuditSqlDialect::Sqlite),
        DatabaseType::ClickHouse => Some(AuditSqlDialect::ClickHouse),
        DatabaseType::DuckDb
        | DatabaseType::H2
        ...
        | DatabaseType::Xugu
        | DatabaseType::Iris
        | DatabaseType::Jdbc => Some(AuditSqlDialect::Ansi),
        _ => None,
    }
}
...
async fn run_document_scan(
    ...
    let databases =
        if let Some(database) = request.database.as_ref().map(|value| value.trim()).filter(|value| !value.is_empty()) {
            ...
            dbx_core::mongo_ops::mongo_list_databases_core(&state, &request.connection_id)
                .await?
                .into_iter()
...
async fn apply_table_connection_meta(state: &AppState, connection_id: &str, table: &mut AuditTableEvidence) {
    let configs = state.configs.read().await;
    let Some(config) = configs.get(connection_id) else {
        return;
    };
...
async fn document_schema(state: &AppState, connection_id: &str) -> String {
...
async fn default_audit_schemas(state: &AppState, connection_id: &str, db_type: Option<DatabaseType>) -> Vec<String> {
    let username = state
        .configs
        .read()
        .await
