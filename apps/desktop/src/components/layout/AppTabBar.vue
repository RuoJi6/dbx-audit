...
import { X, Pin, ChevronDown, Table2, Code2, TableProperties, PencilRuler, KeyRound, Pencil, Package, Lock, Copy, AlertTriangle, ShieldCheck, Network, Minimize2, Maximize2, Settings, CalendarClock, Activity, Gauge } from "@lucide/vue";
...
function tabColorStyle(tab: QueryTab) {
  ...
  const isActive = tab.id === queryStore.activeTabId && !props.driverStoreActive && !props.showAuditPanel && !props.settingsPageActive;
                    ...
                    :class="
                      isClassicLayout
                        ? [
                            ...
                            tab.id === queryStore.activeTabId && !driverStoreActive && !showAuditPanel && !settingsPageActive ? 'bg-background text-foreground' : 'text-foreground/70 hover:text-foreground/90',
                          ]
                        : [
                            compactTabTitle ? 'min-w-24' : 'min-w-38',
                            'h-7 rounded-md border',
                            tab.id === queryStore.activeTabId && !driverStoreActive && !showAuditPanel && !settingsPageActive ? 'text-foreground font-medium' : 'border-border/60 text-foreground/70 hover:border-border hover:text-foreground/90',
                          ]
                    "
                    :style="[tabColorStyle(tab), tabDropStyle(tab.id)]"
                    :data-active-tab="tab.id === queryStore.activeTabId && !driverStoreActive && !showAuditPanel && !settingsPageActive"
          ...
          <PopoverContent align="end" class="w-auto min-w-56 max-w-80 max-h-[min(70vh,28rem)] gap-0 overflow-y-auto rounded-[6px] p-1" @click.stop @keydown.stop>
            <CustomContextMenu v-for="tab in queryStore.tabs" :key="tab.id" :items="getTabMenuItems(tab)" v-slot="{ onContextMenu }">
              <div
                ...
                :class="tab.id === queryStore.activeTabId && !driverStoreActive && !showAuditPanel && !settingsPageActive ? 'bg-accent/70 text-accent-foreground' : ''"
