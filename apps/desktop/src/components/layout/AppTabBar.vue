...
import { X, Pin, ChevronDown, Table2, Code2, TableProperties, PencilRuler, KeyRound, Pencil, Package, Check, Lock, Copy, AlertTriangle, ShieldCheck, Network } from "@lucide/vue";
...
function tabColorStyle(tab: QueryTab) {
  ...
  const isActive = tab.id === queryStore.activeTabId && !props.driverStoreActive && !props.showAuditPanel;
            ...
            <Tooltip>
              <TooltipTrigger as-child>
                <div
                  ...
                  :class="
                    settingsStore.editorSettings.appLayout === 'classic'
                      ? [compactTabTitle ? 'min-w-24' : 'min-w-38', 'h-full border-r border-border/80 font-medium dark:border-border/45', tab.id === queryStore.activeTabId && !driverStoreActive && !showAuditPanel ? 'bg-background text-foreground' : 'text-foreground/70 hover:text-foreground/90']
                      : [compactTabTitle ? 'min-w-24' : 'min-w-38', 'h-7 rounded-md border', tab.id === queryStore.activeTabId && !driverStoreActive && !showAuditPanel ? 'text-foreground font-medium' : 'border-border/60 text-foreground/70 hover:border-border hover:text-foreground/90']
                  "
                  :style="[tabColorStyle(tab), tabDropStyle(tab.id)]"
                  :data-active-tab="tab.id === queryStore.activeTabId && !driverStoreActive && !showAuditPanel"
