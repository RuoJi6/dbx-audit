...
export async function auditExportReport(jobId: string, format: AuditExportFormat, path: string): Promise<AuditExportResult> {
  return invoke("audit_export_report", { jobId, format, path });
}

export async function auditExportReportSnapshot(job: AuditJobState, format: AuditExportFormat, path: string): Promise<AuditExportResult> {
